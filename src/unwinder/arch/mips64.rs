...
pub struct Context {
    ...
    #[cfg(all(not(feature = "soft-float"), not(feature = "mips-single-float")))]
    pub fp: [u64; 32],
    #[cfg(all(not(feature = "soft-float"), feature = "mips-single-float"))]
    pub fp: [u32; 32],
}
...
macro_rules! save {
    (gp64 $(, $fp:ident)?) => {
        core::arch::naked_asm!(
            ...
            .set noat
            move $t0, $sp
            sub $sp, $sp, {ctx_size} + 0x10
            sd $ra, {ctx_size}($sp)
            ",
            maybe_cfi!("
            .cfi_def_cfa_offset {ctx_size} + 0x10
            .cfi_offset $ra, -16
            "),
            "
            sd $s0, 0x80($sp)
            ...
            ctx_size = const mem::size_of::<Context>()
        )
    };
    (maybesavefp()) => { "" };
    (maybesavefp(fp32)) => {
...
pub unsafe fn restore_context(ctx: &Context) -> ! {
    unsafe {
        #[cfg(all(not(feature = "soft-float"), not(feature = "mips-single-float")))]
        restore!(ctx, gp64, fp64);
        #[cfg(all(not(feature = "soft-float"), feature = "mips-single-float"))]
        restore!(ctx, gp64, fp32);
        #[cfg(feature = "soft-float")]
        restore!(ctx, gp64);
    }
}
